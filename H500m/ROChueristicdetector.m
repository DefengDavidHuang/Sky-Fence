...
data_length = 200; %100; %1e3; %1e5; %1e4; %100000; %1e4; %10000; %10000;
...
for i = 1:length(SNR)
    ...
    PD_all(:,i) = 1 - ncx2cdf(chi2inv(1 - PFA,N_D), N_D, lamda(i));
    %temp = 
    PD_matched(:,i) = qfunc(qfuncinv(PFA) - sqrt(lamda(i)));
...
semilogx(PFA', PD_all, 'k.-');
xlabel('Prob of false alarm');
ylabel('Probability of detection (P_{d})');
legend(cellstr(num2str((DNR_dB)', 'DNR (dB) = %0.1f')));
title('Prob of Detection for HueristicDetector (Theoretical)');

save('Theoretical10dB16QAMNA1000.mat','PFA','PD_all')
%grid on;
