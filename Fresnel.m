     ...
     %theta(Num_data_point+1)/pi*180
       ...
       for m = 1:1:Num_data_point+1
        ...
        % funFresnel = @(r1,r2) A*exp(1i * 2 * pi / wavelength * (r1 + r2))./(r1 .* r2) ;
         ...
         temp11_F = sum(exp((1i * pi / (wavelength * dPlane)) * C1 ),'all'); 
         ...
         output_FH(m) = 1 + temp1*(sinc(beta)*sinc(gamma)).^2  - temp2 * (sinc(beta)) * (sinc(gamma)) * sin((pi*((tempY * tempY + 2*z*z)))/(lambda *z));
         tempFH = output_FH(m) * pathloss;
         output_FH(m) = tempFH; %considering free space path loss
         %}
...
% legend('Fresnel Kirchhoff');
 plot(x_time,y_FN_F,'r',x_time,y_FN,'x' );
legend('Fresnel', 'Fresnel Numerical');

%plot(x_time, y_FK, 'b',x_time,y_FH,'c', x_time,y_FN_F,'x');
%legend('Fresnel Kirchhoff','Fraunhofer', 'Fresnel');
