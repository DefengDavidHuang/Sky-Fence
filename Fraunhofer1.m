...
%for m_S = 1:1:Num_satellite
    ...
    % alpha = -1.0 * [start_time : time_step : end_time] * 2 * pi/T_satellite + alpha_initial;   %the alpha angle relative to the first ground station. Note the minus is for, at the beginning the alpha is positive, which corrsponds to negative time
     ...
     %distanceY1st = -1.0 * ([start_time : time_step : end_time]) * plane_speed + plane_height/tan(theta_ini); % relative to the reference ground station (y=0), the minus sign due to the distance at the beginning is set to be positive
      distanceY1st = -1.0 * (start_time + [1 : 1 : (Num_data_point+1)] * time_step) * plane_speed + plane_height/tan(theta_ini);
     %m_S
    ...
    % for m_Y = 1:1:Num_groundstation
     ...
     alpha = -1.0 * (start_time + [1 : 1 : (Num_data_point+1)] * time_step) * 2 * pi/T_satellite + alpha_initial; % - Gstation_location(m_Y)/r_Earth;
     ...
     %theta(Num_data_point+1)/pi*180
       ...
       for m = 1:1:Num_data_point+1
         ...
         dPlane = plane_height/sin(theta(m)) + (distanceY(m)  - plane_height / tan(theta(m))) * cos(theta(m)); %in the optical axis, distance between flying object and groud receiver 
        ...
        % funFresnel = @(r1,r2) A*exp(1i * 2 * pi / wavelength * (r1 + r2))./(r1 .* r2) ;
         ...
         tttemp = 1i*temp11/wavelength * length_x * width_ytemp /Nx /Ny * dLEO * exp(-1i * 2 * pi * dLEO /wavelength); %(dLEO - dPlane) * dPlane % * exp(-1i * 2 * pi * (dLEO - dPlane) /wavelength) * exp(-1i * 2 * pi * dPlane /wavelength); %  %      
         ...
         tttemp_F = 1i*temp11_F/wavelength/dPlane  * length_x * width_ytemp /Nx /Ny; % * dPlane; %* exp(1i * 2 * pi /wavelength * dPlane)
          tempFN = (1 + tttemp_F) * conj(1 + tttemp_F) * pathloss;
          output_FN(m)= tempFN;
         %}
          
          %using Fraunhofer approximation
           ytemp_F = linspace(-width_ytemp/2,width_ytemp/2, N_F);
         % tempFH1 = 1i / (wavelength * dPlane) * sum(exp(-1i * 2 * pi / wavelength / dPlane * (- tempY) * ytemp_F),'all') * exp(1i * pi / (wavelength * dPlane) * tempY * tempY) * width_ytemp /N_F * length_x;
          tempFH1 = 1i * exp(1i * 2 * pi / wavelength * dPlane) / (wavelength * dPlane) * exp(1i * pi / (wavelength * dPlane) * tempY * tempY) * sum(exp(-1i * 2 * pi / (wavelength * dPlane) * (- tempY) * ytemp_F),'all') * width_ytemp /N_F * length_x;
         ...
         F1 = 2 * fresnelc(tt1 * length_x / 2) + 1i * 2 * fresnels(tt1 * length_x / 2);
         F2 = fresnelc(tt1 * (width_ytemp / 2 - tempY)) + fresnelc(tt1 * (width_ytemp / 2 + tempY)) + 1i * fresnels(tt1 * (width_ytemp / 2 - tempY)) + 1i * fresnels(tt1 * (width_ytemp / 2 + tempY));
         ...
         output_FH_F(m) = 1 + temp1*(sinc(beta)*sinc(gamma)).^2  - temp2 * (sinc(beta)) * (sinc(gamma)) * sin((pi*((tempY * tempY + 2*z*z)))/(lambda *z));
 ...
 %    intensity_all_output1(m_S, m_Y,:) = output_intensity;
  ...
  x_time = start_time + [1 : 1 : (Num_data_point+1)] * time_step; %[start_time : time_step : end_time];
...
plot(x_time,y_FH_F','-k', x_time, y_FH',':k');
legend('Fraunhofer', 'Frauhofer Simulation');
...
title(strcat(strcat(num2str(band(wave_select)),strcat(strcat(" GHz    ", num2str(round(theta_initial_degree,2))), ' degree')), strcat("   ", strcat(num2str(plane_speed),' m/sec')))); 
 ...
 save('FH65degree2010fine.mat','x_time','y_FH_F','y_FH');
