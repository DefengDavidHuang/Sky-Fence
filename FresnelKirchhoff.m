...
width_z = 20; %10; %4; %2; %0; %0; %10; %20; %20; %20.0;
       ...
       for m = 1:1:Num_data_point
        ...
        % funFresnel = @(r1,r2) A*exp(1i * 2 * pi / wavelength * (r1 + r2))./(r1 .* r2) ;
         ...
         % tempFH1 = 1i / (wavelength * dPlane) * sum(exp(-1i * 2 * pi / wavelength / dPlane * (- tempY) * ytemp_F),'all') * exp(1i * pi / (wavelength * dPlane) * tempY * tempY) * width_ytemp /N_F * length_x;
          tempFH1 = 1i * exp(1i * 2 * pi / wavelength * dPlane) / (wavelength * dPlane) * exp(1i * pi / (wavelength * dPlane) * tempY * tempY) * sum(exp(-1i * 2 * pi / (wavelength * dPlane) * (- tempY) * ytemp_F),'all') * width_ztemp /N_F * length_x;
         ...
         F2 = fresnelc(tt1 * (width_ztemp / 2 - tempY)) + fresnelc(tt1 * (width_ztemp / 2 + tempY)) + 1i * fresnels(tt1 * (width_ztemp / 2 - tempY)) + 1i * fresnels(tt1 * (width_ztemp / 2 + tempY));
