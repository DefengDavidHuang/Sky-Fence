...
plane_speed = 250; %100; %250; %-125% -250; %-600; %-50; %600; %250; %0; % %-600; %-50; %  -250; % %600; % %600; % % %600; %200;   0; %50; % %250m/second = 900km/hour
...
time_step = 0.00051; %0.0000051; %05;
total_time = 0.3; %5e-3; % %0.1; %0.6; %0.6; %0.0006; % % second  
start_time = -0.15; %-0.05; % %-0.05; %0.3; %-0.3; %-0.0003; % %
...
width_z = 20; %20; %10; %4; %2; %0; %0; %10; %20; %20; %20.0;
length_x = 10; %10; %10; %4; % 2; %0; %10; %0; %20; %20.0;    %x: parallel to ground and perpendicular to the satellite link
...
distanceX1st = [start_time : time_step : end_time] * plane_speed; %-1.0 * ([start_time : time_step : end_time]) * plane_speed; 
...
%alpha = -0.0 * [start_time : time_step : end_time] * 2 * pi/T_satellite + alpha_initial;
...
for m = 1:1:Num_data_point
     ...
     tetemp = 2 * pi/ wavelength * (1/dPlane + 1/(dLEO - dPlane));
     eta(m) = tetemp;
     ...
     R_LEO = sqrt(C1 + dLEO2);   %distance with satellite
         ...
         %temp11 = sum(exp(1i * 2 * pi / wavelength * (R_Station + R_LEO))./(R_Station .* R_LEO),'all') * length_x * width_ztemp /Nx /Ny;  
     temp1123 = sum(exp(1i * 2 * pi / wavelength * (R_Station + R_LEO))./(R_Station .* R_LEO),'all') * length_x * width_ztemp /Nx /Ny;
     temp11 = temp1123 * dPlane * (dLEO - dPlane); % phase to be considered later
         
     output_FK(m) = temp11;
         
         %tttemp = 1i*temp11/wavelength  * dLEO * exp(-1i * 2 * pi * dLEO /wavelength); %(dLEO - dPlane) * dPlane % * exp(-1i * 2 * pi * (dLEO - dPlane) /wavelength) * exp(-1i * 2 * pi * dPlane /wavelength); %  %      
 ...
 % y_FK = real(output_FK);
  ...
  save('signal1.mat','x_time','output_FK','eta');
  
  gamma_prime = median(eta)*plane_speed
  
  Xomega = fftshift(fft(fftshift(output_FK))) * time_step;  %note about phase shifting
  
  ome = ([0:1:length(x_time)-1]-round(length(x_time)/2))*2*pi/time_step/length(x_time);  % using FFT to compute X(omega)
  xome = ome / gamma_prime;
  
  figure
  plot(xome', real(Xomega), '-k');
  
  figure
  plot(xome', imag(Xomega), '-xk');
  
  % retrieval of upper bound and lower bound
  y1 = real(Xomega);
  y2 = imag(Xomega);
  kapp1k = -1 * ones(length(y1),1);
  kapp2k = 1.5 * ones(length(y1),1);
  %kapp2k = zeros(length(y1),1);
 ...
 % plot(xome', kapp1k, '-k');
  ...
  for k = 1:1:5
      ...
      figure
      plot(xome', kapp2k, '-xk');
      temp1 = fresnelc(kapp2k) - fresnelc(kapp1k) - y1;
      temp2 = fresnels(kapp2k) - fresnels(kapp1k) - y2;
  ...
  %Numerical result regarding Xomega
  %if t=0, then 
  %{
  kapp1 = - sqrt(pi /median(eta)) * width_z/2;
  kapp2 = sqrt(pi /median(eta)) * width_z/2;
  XomegaNum = sqrt(pi/2/median(eta)) * ((fresnelc(kapp2) - fresnelc(kapp1)) + 1i * (fresnels(kapp2) - fresnels(kapp1)))
  abs(XomegaNum)
  %}
...
% save('FK65degree2010cross90degree0mps.mat','x_time','y_FK','y_FH_F','y_FH','y_FN_F','y_FN');
